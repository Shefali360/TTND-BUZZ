...
const {invalidTokenError}=require('../ErrorHandler/authExceptions');
const {CustomExceptionts}=require('../ErrorHandler/exceptionModel');
const {authHeadersAbsent,invalidAuthHeaderFormat,authTokenAbsent}=require('../ErrorHandler/authExceptions');
...
module.exports.verifyTokenMiddleware=async(req, res, next)=> {
  try{if(!req.headers['authorization']){
    return next(new authHeadersAbsent('Authorization headers are absent',401));
  }
    const tokenType= req.headers['authorization'].split(' ')[0];
    if(tokenType!=='Bearer'){
      return next(new invalidAuthHeaderFormat("Auth token should be of bearer type",401));
    }
  ...
  module.exports.verifyTokenToGetUserData=async(req,res,next)=>{
    ...
    try{
     req.data= await axios.get('https://oauth2.googleapis.com/tokeninfo' + `?id_token=${idToken}`);
      return next();
  } catch (err) {
    return next(new invalidTokenError("Invalid id token received",401,err['response']['data']));
  }
}catch{
  return next(new ServerError("Error"),500);
}
